    ...
    return (Self = nit.defineClass ("postgresql.QueueServer"))
        ...
        .do ("Task", Task =>
        {
            Task
                ...
                {
                    Method
                        .after ("before", "beginTransaction", async function (task, ctx)
                        {
                            await ctx.db;
                        ...
                        .before ("after", "endTransaction", async function (task, ctx)
                        {
                            if (task.constructor.transactional)
                            {
                                await ctx.db.commit ();
                        ...
                        .afterFailure ("before", "rollbackTransaction", async function (task, ctx)
                        {
                            if (task.constructor.transactional)
                            {
                ...
                {
                    Method.before ("cancelTimer", task => task.timer?.cancel ());
                })
        ...
        {
            Method
                .before ("createJobTable", async function ({ Job, db, channel, notificationListener })
                {
                ...
                .after ("updateEnqueueTimer", server => server.updateEnqueueTimer ())
            ;
        })
        .configureComponentMethod ("stop", Method =>
        {
            Method
                .before ("initArgs", nit.typedFunc ({ server: Self, graceful: "boolean" }, function (server, graceful = false)
                {
                    this.args = [graceful];
                }))
                .after ("disconnectDatabase", async function ({ taskQueue, db, channel, notificationListener }, graceful)
                {
