module.exports = function (nit, postgresql, Self)
{
    return (Self = nit.defineClass ("postgresql.QueueServer"))
        ...
        .k ("createJobTable", "initArgs", "disconnectDatabase", "updateEnqueueTimer")
        .use ("nit.Command")
        ...
        .plugin ("server")
        .plugin ("task-queue")
        .field ("<jobModel>", "string", "The job model name.", "postgresql.models.Job")
            .constraint ("subclass", "postgresql.models.Job", true)
            ...
            Task
                ...
                .defineContext (Context =>
                {
                    Context
                        ...
                        .field ("job", "postgresql.models.Job")
        ...
        {
            return new Self.DatabaseProvider (this);
        })
        ...
        {
            Method
                .after (Self.kStopIfStarted, Self.kCreateJobTable, async function ({ Job, db, channel, notificationListener })
                {
                ...
                .after (Self.kUpdateEnqueueTimer, server => server.updateEnqueueTimer ())
            ;
        })
        .configureComponentMethod ("stop", Method =>
        {
            Method
                .before (Self.kInitArgs, nit.typedFunc ({ server: Self, graceful: "boolean" }, function (server, graceful = false)
                {
                    this.args = [graceful];
                }))
                .before (Self.kReturnServer, Self.kDisconnectDatabase, async function ({ taskQueue, db, channel, notificationListener }, graceful)
                {
        ...
        .defineQueuedTaskMethod ("runJob", RunJob =>
        {
            RunJob
                ...
                .onRunComplete (async function (c)
                {
