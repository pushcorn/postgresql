    ...
    return (Self = nit.defineModel ("postgresql.Model"))
        ...
        .defineInnerClass ("ActionContext", ActionContext =>
        {
            ActionContext
                ...
                .field ("matches", "object", "Entity match conditions.")
                ...
                .method ("updateValues", async function ()
                {
                    let ctx = this;
                    let cls = ctx.new.constructor;

                    ctx.options.entities.marshalled = {};
                    ctx.values = nit.pick (await cls.marshall (ctx.new, ctx), cls.table.columnNames);
                    ctx.values = nit.omit (ctx.values, nit.keys (ctx.matches));

                    return ctx;
                })
                .method ("updateChanges", async function ()
                {
                    let ctx = this;
                    let cls = ctx.new.constructor;

                    await ctx.updateValues ();

                    ctx.changes = nit.pick (await cls.marshall (ctx.old), cls.table.columnNames);
                    ctx.changes = nit.omit (ctx.changes, nit.keys (ctx.matches));
        ...
        .staticMethod ("create", async function (...args)
        {
            let cls = this;
        ...
        .configureComponentMethod ("update", Method =>
        {
            Method
                ...
                .after (Self.kValidateEntity, Self.kBuildActionContext, async function (entity, data, options)
                {
                    ...
                    let ctx = new Self.ActionContext ("update", cls.db, { new: entity, old, options, matches });

                    this.args = await ctx.updateChanges ();
                })
                .after ("update.invokeHook", Self.kUpdateRow, async function (entity, ctx)
                {
                    let cls = entity.constructor;

                    await ctx.updateChanges ();
        ...
        .configureComponentMethod ("insert", Method =>
        {
            Method
                ...
                .after (Self.kValidateEntity, Self.kBuildActionContext, async function (entity, options)
                {
                    ...
                    this.args = await ctx.updateValues ();
                })
