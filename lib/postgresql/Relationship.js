module.exports = function (nit, postgresql, Self)
{
    return (Self = nit.defineClass ("postgresql.Relationship"))
        ...
        {
            const { ownerClass, referencedClass, joinModelName, ownerField, referencedField } = r;
            ...
            {
                let on = referencedField ? nit.singularize (referencedField.name) : nit.lcFirst (ownerClass.simpleName);
                let rn = nit.singularize (ownerField.name);
                ...
                ownerClass.classChain
                    ...
                    .field ("<" + on + ">", ownerClass.name, { key: true, relType: "manyToOne" })
                    .field ("<" + rn + ">", referencedClass.name, { key: true, relType: "manyToOne" })
                ;
            }
        ...
        .staticMethod ("new", function (field)
        {
            ...
            if (!ownerField.relType)
            {
                if (ownerField.array)
                {
                    if (referencedField && !referencedField.array)
                    {
