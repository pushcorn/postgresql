    ...
    return (Self = nit.defineClass ("postgresql.Database"))
        ...
        .defineInnerClass ("Registry", "postgresql.registries.Cached", Registry =>
        {
            Registry
                ...
                {
                    let self = this;
                    let Model = Registry.superclass.prototype.lookup.call (self, name, optional);

                    return Model ? (Model.db = self.db) && Model : undefined;
                })
            ;
        })
        ...
        .defineLogger (Logger =>
        {
            Logger
                ...
                {
                    let poolId = db.pooling ? `.${db.pooling}` : "";

                    return `[${level.toUpperCase ()}] ` + ((level == "debug" || level == "error") ? `(${db.constructor.name}@${db.id}${poolId}) ` : "");
                })
            ;
        })
        ...
        .typedMethod ("select",
            ...
            async function (table, matches, otherClauses, query)
            {
                ...
                if (matches) { nit.each (matches, (v, k) => (k.length ? query.Where (k, v) : nit.array (v).forEach (v => query.WhereExpr (v)))); }
