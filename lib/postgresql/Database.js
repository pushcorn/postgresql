module.exports = function (nit, postgresql, Self)
{
    ...
    return (Self = nit.defineClass ("postgresql.Database"))
        ...
        .staticMemo ("logger", function ()
        {
            return this.lookupPlugin ("plugins.Logger");
        })
        ...
        .onLogLevelPrefix (function (level, db)
        {
            return `[${level.toUpperCase ()}] ` + (level == "debug" ? `(${this.name}@${db.id}) ` : "");
        })
        .onLogLevelColor (function (level, db)
        {
            return this.logger.levelColors[level] || db.logColor;
        })
